...
function getSupabaseInstance() {
  ...
  try {
    ...
    logger.warn('⚠️ Supabase client not yet available:', error);
    return null;
  }
}
...
export async function listUserRegistrationRecords(filters?: {
  ...
  logger.info('📊 Fetching users from users table as fallback');
...
export async function approveUserRegistrationRecord(
  ...
  if (!registration) {
    logger.info('📊 Registration not found in user_registrations, checking users table');
  ...
  if (registration) {
    ...
    if (error) logger.error('Failed to update user_registrations:', error);
  }
...
export class SupabaseDataStore implements DataStore {
  ...
  private async initializeAuthSession(authSession: SupabaseAuthSessionPayload) {
    try {
      ...
      if (error) {
        logger.error('Failed to establish Supabase session:', error);
      }
    } catch (error) {
      logger.error('Unexpected Supabase auth error:', error);
    }
  }
  ...
  private async _fetchProfileFromDatabase(): Promise<User> {
    ...
    if (this.authInitialization) {
      try {
        ...
        logger.error('getProfile: Auth initialization failed:', error);
      }
    }
    ...
    if (sessionError || !sessionData?.session) {
      logger.error('❌ getProfile: No authenticated session:', sessionError);
    ...
    if (error) {
      logger.error('❌ getProfile: Database error:', error);
    ...
    if (!data) {
      logger.error('❌ getProfile: User record not found for auth.uid():', authUid);
    ...
    if (telegramUserData && (telegramUserData.first_name || telegramUserData.username || telegramUserData.photo_url)) {
      ...
      if (Object.keys(updates).length > 0) {
        ...
        if (updateError) {
          logger.error('⚠️ getProfile: Failed to update user data:', updateError);
    ...
    if (!data || !data.id) {
      logger.error('❌ getProfile: User data missing id field:', data);
  ...
  async getCurrentRole(): Promise<User['role'] | null> {
    if (!this.userTelegramId) {
      logger.warn('getCurrentRole: No userTelegramId provided');
      return null;
    }

    // Wait for auth session to be established if it's in progress
    if (this.authInitialization) {
      logger.info('getCurrentRole: Waiting for auth initialization...');
      try {
        await this.authInitialization;
        logger.info('getCurrentRole: Auth initialization complete');
      } catch (error) {
        logger.error('getCurrentRole: Auth initialization failed:', error);
      }
    }

    logger.info(`🔍 getCurrentRole: Fetching role for telegram_id: ${this.userTelegramId}`);
    ...
    logger.info('🔍 getCurrentRole: RAW RESPONSE:', {
      hasData: !!data,
      hasError: !!error,
    ...
    if (error) {
      logger.error('❌ getCurrentRole: Database error:', error);
      throw error;
    }

    if (!data) {
      logger.warn('⚠️ getCurrentRole: No user found in database (RLS blocked?)');
      return null;
    }

    logger.info(`✅ getCurrentRole: Successfully fetched role: ${data.role}`);
    this.user = data;
    return data.role;
  }

  clearUserCache(): void {
    logger.info('🗑️ clearUserCache: Clearing cached user data and timestamp');
    this.user = null;
  ...
  private async ensureUserExists(): Promise<void> {
    ...
    logger.info(`✅ Created user record for ${this.userTelegramId}`);
  }
  ...
  async createBusiness(input: {
    ...
    logger.info('🔄 createBusiness: Starting...', input);
    ...
    logger.info('✅ Using default infrastructure:', infrastructureId);
    ...
    if (typeError) {
      logger.error('❌ Failed to lookup business type:', typeError);
      throw new Error('Failed to lookup business type');
    }

    const businessTypeId = businessType?.id || null;
    logger.info('✅ Business type ID:', businessTypeId);
    ...
    if (error) {
      logger.error('❌ createBusiness: Error inserting:', error);
      throw error;
    }

    logger.info('✅ createBusiness: Created business:', data.id);

    // Wait for database triggers to complete and verify business role was created
    logger.info('⏳ Waiting for database triggers to complete...');
    await new Promise(resolve => setTimeout(resolve, 1000));

    // Verify business role was created by trigger before proceeding
    logger.info('🔍 Verifying business role was created by trigger...');
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      ...
      if (roleCheck && roleCheck.is_active) {
        logger.info('✅ Business role verified:', roleCheck);
        roleVerified = true;
        break;
      }

      if (attempt < 4) {
        logger.info(`⏳ Business role not ready yet, waiting... (attempt ${attempt + 1}/5)`);
        await new Promise(resolve => setTimeout(resolve, 500 * (attempt + 1)));
      }
    }

    if (!roleVerified) {
      logger.warn('⚠️ Business role not found after trigger wait - using link_user_to_business as fallback');

      // Use the new link_user_to_business function to ensure proper setup
      try {
        logger.info('🔄 Calling link_user_to_business to ensure proper setup...');
        ...
        if (linkError) {
          logger.error('❌ link_user_to_business failed:', linkError);
        } else {
          logger.info('✅ link_user_to_business succeeded:', linkResult);
          roleVerified = true;
        }
      } catch (linkError) {
        logger.error('❌ link_user_to_business error:', linkError);
      }
    }

    // Step 1: Set active business context using the new set_user_active_business function
    logger.info('🔄 Setting active business context...');
    ...
    try {
      ...
      if (contextError) {
        logger.warn('⚠️ set_user_active_business failed:', contextError);
        logger.info('ℹ️ Will try fallback methods');
      } else {
        logger.info('✅ Business context set successfully:', contextResult);
        contextSwitchSucceeded = true;
      }
    } catch (contextError) {
      logger.warn('⚠️ set_user_active_business error:', contextError);
      logger.info('ℹ️ Will try fallback methods');
    }
      ...
      try {
        ...
        if (sessionData?.session?.access_token) {
          ...
          if (!switchResponse.error) {
            logger.info('✅ Context switched via fallback edge function');
            contextSwitchSucceeded = true;
          }
        }
      } catch (fallbackError) {
        logger.warn('⚠️ Fallback context switch also failed:', fallbackError);
      }
    }
    ...
    const syncJwtClaims = async (retryCount = 0): Promise<boolean> => {
      ...
      try {
        logger.info(`🔄 Syncing JWT claims (attempt ${retryCount + 1}/${maxRetries + 1})...`);
        ...
        if (syncResponse.error) {
          logger.error('❌ JWT claims sync failed:', syncResponse.error);
          logger.error('❌ Response data:', syncResponse.data);
        ...
        logger.info('✅ JWT claims synced successfully:', syncResponse.data);
        return true;
      } catch (syncError) {
        logger.error('❌ JWT claims sync error:', syncError);
    ...
    if (!syncSuccess) {
      logger.warn('⚠️ JWT claims sync failed after retries - business created but permissions may be delayed');
    }
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        logger.info(`🔄 Refreshing user session (attempt ${attempt + 1}/3)...`);
        const { data: sessionData, error: refreshError } = await supabase.auth.refreshSession();

        if (refreshError) {
          logger.warn(`⚠️ Session refresh attempt ${attempt + 1} failed:`, refreshError);
          if (attempt < 2) {
        ...
        logger.warn(`⚠️ Session refresh error (attempt ${attempt + 1}):`, refreshError);
        if (attempt < 2) {
          await new Promise(resolve => setTimeout(resolve, 1000 * (attempt + 1)));
        }
      }
    ...
    if (!sessionRefreshed) {
      logger.warn('⚠️ Failed to refresh session after 3 attempts');
      logger.info('ℹ️ Business created successfully, but session may need manual refresh');
    ...
    logger.info('🔄 Querying business role from database...');
    ...
    while (!businessRole && roleQueryAttempts < maxRoleQueryAttempts) {
      ...
      try {
        logger.info(`🔄 Querying business role (attempt ${roleQueryAttempts}/${maxRoleQueryAttempts})...`);
        ...
        if (roleError) {
          logger.error('❌ Failed to query business role:', roleError);
          ...
          logger.info('✅ Business role found:', roleData);
          businessRole = roleData;
          break;
        } else {
          logger.warn(`⚠️ No business role found yet (attempt ${roleQueryAttempts}) - waiting for triggers...`);
        ...
        logger.error('❌ Error querying business role:', roleQueryError);
    ...
    if (businessRole) {
      ...
      logger.info('💾 Business role cached in localStorage for immediate UI access');
    } else {
      logger.warn('⚠️ Business role not found after all retries - will be loaded on next page refresh');
    }
    ...
    // Final summary
    logger.info('🎉 Business creation flow complete!');
    logger.info('📊 Summary:', {
      businessId: data.id,
    ...
    if (!sessionRefreshed || !businessRole) {
      logger.info('ℹ️ Note: Some steps were incomplete. The business was created successfully.');
      logger.info('ℹ️ If you don\'t see the business immediately, please refresh the page.');
    }
  ...
  async createBusinessType(input: Omit<BusinessType, 'id' | 'created_at' | 'updated_at' | 'created_by'>): Promise<{ id: string }> {
    logger.info('🔄 createBusinessType: Starting...', input);
    ...
    if (userError) {
      logger.error('❌ createBusinessType: Error fetching user:', userError);
      throw userError;
    }

    if (!user) {
      logger.error('❌ createBusinessType: User not found for telegram_id:', this.userTelegramId);
      throw new Error('User not found');
    }

    logger.info('✅ createBusinessType: Found user ID:', user.id);
    ...
    if (error) {
      logger.error('❌ createBusinessType: Error inserting:', error);
      throw error;
    }

    logger.info('✅ createBusinessType: Created business type:', data.id);
    return { id: data.id };
  }
  ...
  async listAllUsers(): Promise<any[]> {
    logger.info('📋 listAllUsers: Fetching all users from database...');

    const { data, error } = await supabase
      .from('users')
      .select('id, telegram_id, name, username, photo_url, role, phone, created_at, updated_at, wallet_address_eth, wallet_address_sol, auth_method')
      .order('name', { ascending: true });

    if (error) {
      logger.error('❌ listAllUsers: Error fetching users:', error);
      throw error;
    }

    logger.info(`✅ listAllUsers: Loaded ${data?.length || 0} users from database:`, data);
    return data || [];
  }
  ...
  async getUserBusinesses(): Promise<any[]> {
    try {
      ...
      logger.error('getUserBusinesses: Error accessing Supabase:', error);
      throw error;
    }
  }
  ...
  async getActiveBusinessContext(): Promise<any | null> {
    ...
    if (error) {
      logger.error('Failed to get active business context:', error);
      return null;
    }

    return data;
    } catch (error) {
      logger.error('getActiveBusinessContext: Error accessing Supabase:', error);
      return null;
    }
  }
  ...
  async setActiveBusinessContext(business_id: string): Promise<void> {
    try {
      ...
      if (error) {
        logger.error('Failed to set active business context:', error);
        throw error;
      }
    } catch (error) {
      logger.error('setActiveBusinessContext: Error accessing Supabase:', error);
      throw error;
    }
  }
  ...
  async listAllUsersForMessaging(): Promise<User[]> {
    logger.info('📋 listAllUsersForMessaging: Fetching all users for messaging...');
    const supabase = getSupabaseInstance();
    if (!supabase) {
      logger.warn('Supabase client not available');
      return [];
    ...
    if (error) {
      logger.error('❌ listAllUsersForMessaging: Error fetching users:', error);
      throw error;
    }

    logger.info(`✅ listAllUsersForMessaging: Loaded ${data?.length || 0} users from database`);
    return (data || []).map((row: any) => ({
      telegram_id: row.telegram_id,
      role: row.role,
      name: row.name,
