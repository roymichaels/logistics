...
export class DriverAssignmentService {
  ...
  async findBestDriver(
    ...
    try {
      ...
      logger.error('Failed to find best driver:', error);
      return {
        success: false,
        reason: 'error',
        alternatives: []
  ...
  async assignOrderToDriver(
    ...
    try {
      ...
      logger.error('Failed to assign order to driver:', error);
      return false;
    }
  }
  ...
  async getDriversInZones(zoneIds: string[]): Promise<string[]> {
    try {
      ...
      logger.error('Failed to get drivers in zones:', error);
      return [];
    }
  }
  ...
  async predictDriverAvailability(
    ...
    try {
      ...
      logger.error('Failed to predict availability:', error);
      return {
        available: false,
        confidence: 0,
        reason: 'שגיאה בחיזוי'
  ...
  async getOptimalAssignmentTime(orderId: string): Promise<{
    ...
    try {
      ...
      logger.error('Failed to get optimal assignment time:', error);
      return {
        immediate: true,
