...
class TelegramService {
  ...
  private async loadSDK() {
    ...
    try {
      ...
      logger.info('🌐 Telegram SDK not available - running in standard web mode');
  ...
  setMainButton({ text, visible = true, onClick }: { text: string; visible?: boolean; onClick: () => void }): void {
    ...
    try {
      ...
      logger.info('[Telegram] MainButton error:', error);
    }
  }
  ...
  hideMainButton(): void {
    ...
    try {
      ...
      logger.info('[Telegram] MainButton hide error:', error);
    }
  }
  ...
  setBackButton(onClick: () => void): void {
    ...
    try {
      ...
      logger.info('[Telegram] BackButton not supported in this version');
    }
  }
  ...
  hideBackButton(): void {
    ...
    try {
      ...
      logger.info('[Telegram] BackButton hide error:', error);
    }
  }
  ...
  hapticFeedback(
    ...
    try {
      ...
      logger.info('[Telegram] HapticFeedback not supported in this version');
    }
  }
  ...
  showAlert(message: string): void {
    if (this.isAvailable) {
      try {
        ...
        logger.info('[Telegram] showAlert not supported, using fallback');
        alert(message);
      }
    } else {
      alert(message);
    ...
    return new Promise((resolve) => {
      if (this.isAvailable) {
        try {
          ...
          logger.info('[Telegram] showConfirm not supported, using fallback');
    ...
    return new Promise((resolve) => {
      if (this.isAvailable) {
        try {
          ...
          logger.info('[Telegram] showPopup not supported, using alert fallback');
  ...
  openLink(url: string, options?: { try_instant_view?: boolean }): void {
    if (this.isAvailable) {
      try {
        ...
        logger.info('[Telegram] openLink error:', error);
  ...
  openTelegramLink(url: string): void {
    if (this.isAvailable) {
      try {
        ...
        logger.info('[Telegram] openTelegramLink error:', error);
        window.open(url, '_blank');
      }
    } else {
      window.open(url, '_blank');
  ...
  requestContact(): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        logger.info('[Telegram] requestContact not supported');
        reject(error);
      }
    });
  }
  ...
  enableClosingConfirmation(): void {
    if (this.isAvailable) {
      try {
        ...
        logger.info('[Telegram] enableClosingConfirmation not supported');
      }
    }
  }
  ...
  disableClosingConfirmation(): void {
    if (this.isAvailable) {
      try {
        ...
        logger.info('[Telegram] disableClosingConfirmation not supported');
      }
    }
  }
  ...
  onEvent(eventType: string, handler: () => void): void {
    if (this.isAvailable) {
      try {
        ...
        logger.info(`[Telegram] Event ${eventType} not supported`);
      }
    }
  }
