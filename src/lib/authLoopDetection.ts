...
export function recordAuthAttempt(type: 'login' | 'logout', userId?: string): void {
  try {
    ...
    console.log(`🔐 Auth attempt recorded: ${type}`, {
...
function getAuthAttempts(): AuthAttempt[] {
  try {
    ...
    console.error('Failed to parse auth attempts:', error);
    return [];
  }
}
...
export function isInAuthLoop(): boolean {
  try {
    ...
    if (recentAttempts.length >= MAX_ATTEMPTS_IN_WINDOW) {
      console.warn(`⚠️ Auth loop detected: ${recentAttempts.length} attempts in ${LOOP_DETECTION_WINDOW_MS / 1000}s`);
      return true;
    }
    ...
    if (recentAttempts.length >= 4) {
      ...
      if (alternatingCount >= 3) {
        console.warn(`⚠️ Auth loop detected: rapid login/logout alternation`);
        return true;
      }
    }
    ...
    console.error('Failed to check auth loop:', error);
    return false;
  }
}
...
export function activateCircuitBreaker(): void {
  try {
    ...
    console.warn(`🚨 Circuit breaker activated until ${new Date(cooldownUntil).toISOString()}`);
  } catch (error) {
...
export function isCircuitBreakerActive(): boolean {
  try {
    ...
    if (now < cooldownUntil) {
      ...
      console.log(`⏸️ Circuit breaker active for ${remainingSeconds}s more`);
      return true;
    }
...
export function resetAuthLoopDetection(): void {
  try {
    ...
    localStorage.removeItem(CIRCUIT_BREAKER_KEY);
    console.log('✅ Auth loop detection reset');
  } catch (error) {
    console.error('Failed to reset auth loop detection:', error);
  }
}
