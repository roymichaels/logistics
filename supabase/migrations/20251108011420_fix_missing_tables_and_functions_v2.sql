/*
  ...
  2. Missing Functions
    ...
    - `get_user_by_telegram_id` - Safe user lookup helper
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'group_chats' AND policyname = 'Users can view group chats they are members of'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'group_chats' AND policyname = 'Authenticated users can create group chats'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'group_chats' AND policyname = 'Group chat creators and members can update'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'user_presence' AND policyname = 'Users can view all user presence'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'user_presence' AND policyname = 'Users can insert their own presence'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'user_presence' AND policyname = 'Users can update their own presence status'
  ) THEN
